...
class USAiGovLLM {
  constructor(embedder = null, modelPreference = null) {
    ...
    if (!process.env.USAI_GOV_BASE_PATH)
      throw new Error(
        "USAi.gov LLM must have a valid base path to use for the api."
      );

    this.className = "USAiGovLLM";
    this.basePath = process.env.USAI_GOV_BASE_PATH;
    this.openai = new OpenAIApi({
      baseURL: this.basePath,
      apiKey: process.env.USAI_GOV_API_KEY ?? null,
    });
    this.model = modelPreference ?? process.env.USAI_GOV_MODEL_PREF ?? null;
    this.maxTokens = process.env.USAI_GOV_MAX_TOKENS
      ? toValidNumber(process.env.USAI_GOV_MAX_TOKENS, 1024)
      : 1024;
    if (!this.model)
      throw new Error("USAi.gov LLM must have a valid model set.");
    this.limits = {
  ...
  static promptWindowLimit(_modelName) {
    const limit = process.env.USAI_GOV_MODEL_TOKEN_LIMIT || 4096;
